...
app.get("/api/slob_knjige", (req, res) => {
  const query = `
  SELECT 
  (knjige.stanje - COUNT(rezervacije.knjiga_id)) AS slobodne, 
  knjige.id, knjige.naslov, knjige.stanje 
  FROM knjige 
  LEFT JOIN rezervacije ON knjige.id = rezervacije.knjiga_id 
  GROUP BY knjige.id`;
...
app.get("/api/slob_knjige/:id_knjige", (req, res) => {
  ...
  const query = `
  SELECT (knjige.stanje - COUNT(rezervacije.knjiga_id)) AS slobodne 
  FROM knjige 
  LEFT JOIN rezervacije ON knjige.id = rezervacije.knjiga_id 
  WHERE knjige.id = ? 
  GROUP BY knjige.id`;
...
app.get("/api/rezerv_knjige", (req, res) => {
  const query = `
  SELECT * 
  FROM knjige, rezervacije 
  WHERE knjige.id = rezervacije.knjiga_id`;
  connection.query(query, (error, results) => {
    if (error) throw error;
    res.send(results);
  });
...
app.get("/api/rezerv_knjige_korisnici", (req, res) => {
  const query = `
  SELECT * 
  FROM knjige, rezervacije, korisnici 
  WHERE knjige.id = rezervacije.knjiga_id 
  AND korisnici.id = rezervacije.korisnik_id`;
  connection.query(query, (error, results) => {
    if (error) throw error;
    res.send(results);
  });
...
app.post("/api/rezerv_knjige/:knjiga_id", (req, res) => {
  ...
  const query = `
  INSERT INTO rezervacije (datum_rezervacije, knjiga_id, korisnik_id) 
  VALUES (?, ?, ?)`;
...
app.delete("/api/rezerv_knjige/:knjiga_id", (req, res) => {
  const knjiga_id = req.params.knjiga_id;
  connection.query(
    "DELETE FROM rezervacije WHERE knjiga_id = ?",
    [knjiga_id],
    (error, results) => {
      if (error) throw error;
      res.send(results);
...
app.get("/api/korisnici_za_knjigu/:id_knjige", (req, res) => {
    ...
    const query = `
      ...
      FROM korisnici
      JOIN rezervacije ON korisnici.id = rezervacije.korisnik_id
      WHERE rezervacije.knjiga_id = ?;
    `;
...
app.get("/api/ukupno_slobodno", (req, res) => {
    const query = `
      SELECT SUM(knjige.stanje - COALESCE(rezervacije.rezervirano, 0)) AS slobodno_primjeraka
      FROM knjige
      LEFT JOIN (
        SELECT knjiga_id, COUNT(*) AS rezervirano
        FROM rezervacije
        GROUP BY knjiga_id
      ) AS rezervacije ON knjige.id = rezervacije.knjiga_id;
    `;
...
app.get("/api/korisnici_duze_od_mjesec", (req, res) => {
    const query = `
      SELECT korisnici.id, korisnici.ime, korisnici.prezime, korisnici.email, korisnici.broj_telefona, 
             rezervacije.knjiga_id, rezervacije.datum_rezervacije
      FROM korisnici
      JOIN rezervacije ON korisnici.id = rezervacije.korisnik_id
      WHERE rezervacije.datum_rezervacije < CURDATE() - INTERVAL 1 MONTH;
    `;
  ...
  // API:(11. zadatal) Dohvat emaila, broja telefona, naslova knjige i datuma rezervacije za korisnike s rezervacijama duže od mjesec dana
app.get("/api/kontakt_duge_rezervacije", (req, res) => {
    const query = `
      SELECT korisnici.email, korisnici.broj_telefona, knjige.naslov, rezervacije.datum_rezervacije
      FROM korisnici
      JOIN rezervacije ON korisnici.id = rezervacije.korisnik_id
      JOIN knjige ON rezervacije.knjiga_id = knjige.id
      WHERE rezervacije.datum_rezervacije < CURDATE() - INTERVAL 1 MONTH;
    `;
...
app.get("/api/rezervacije_iste_knjige/:korisnik_id", (req, res) => {
    const korisnik_id = req.params.korisnik_id;
    ...
    const query = `
      ...
      GROUP BY rezervacije.knjiga_id
